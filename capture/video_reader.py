...
class VideoFrameReader:
    ...
    ``cv2.VideoCapture`` and decodes **forward**: when the requested frame is
    at or after the current position it reads sequentially (no seek), and only
    ...
    (the same cost as the previous open/seek/read/close implementation).

    Frames returned are byte-identical to that previous implementation: same
    decoder, same color order (BGR), same dtype.

    Not thread-safe (single shared capture + position cursor). Callers needing
    concurrent access to one source should use a separate reader per thread.
    """
    ...
    def __init__(self, video_path: str, start: int = None, end: int = None):
        ...
        # Persistent capture for sequential forward decode. Lazily opened on
        # first read; ``_cap_pos`` is the global index of the NEXT frame the
        # capture will return (i.e. one past the last decoded frame).
        self._cap = None
        self._cap_pos = -1
        logger.info(
    ...
    def read_bgr(self, local_idx: int) -> np.ndarray:
        ...
        requested global index is behind the current cursor.
        """
        import cv2
        ...
        # reads sequentially, skipping intervening frames without seeking.
